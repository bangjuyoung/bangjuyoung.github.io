...
│   │   │   │   └── post-service.ts
│   │   │   ├── infrastructure/ # 🧱 인프라 레이어 (리포지토리 구현)
...
│   │   ├── domain/         # 🎯 공유 도메인 개념 (값 객체)
│   │   │   ├── locale.ts   # Locale 값 객체
│   │   │   └── slug.ts     # Slug 값 객체
│   │   ├── lib/            # 🧩 공유 유틸리티
│   │   │   └── utils.ts
│   │   ├── config/         # 공유 설정
│   │   │   └── site-metadata.ts # 블로그 제목, 설명 등
│   │   ├── ui/             # 🖼️ 공유 UI 컴포넌트 (Shadcn/ui 및 기본 요소)
│   │   │   ├── shadcn-ui/  # shadcn/ui 컴포넌트 재수출 및 확장
│   │   │   │   └── button.tsx
│   │   │   └── base-elements/ # 아주 기본적인 UI 요소
│   │   │       └── icon.tsx
...
- `infrastructure/`: 실제 데이터 저장, 외부 API, 파일 접근 등 도메인 인터페이스의 구체적 구현
...
- 여러 도메인에 걸쳐 사용되는 **공통 기능(횡단 관심사)**을 모듈화 (예: Analytics, Comments, AdSense, i18n 등).
...
- `api/` 라우트에서는 `infrastructure` 계층 구현체 사용

#### shared/ (Shared Kernel)

- 전역적으로, 어떤 도메인에도 강하게 종속되지 않는 재사용성 높은 코드 집합
- `domain/`: 여러 도메인에서 쓰는 값 객체 (예: Locale, Slug)
- `lib/`: 범용 유틸리티(예: 날짜 포맷터)
- `config/`: 전역 설정, 사이트 메타데이터 등
- `ui/`: shadcn/ui 확장, 아이콘 등 기본 UI 요소
